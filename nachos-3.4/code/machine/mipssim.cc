...
Machine::OneInstruction(Instruction *instr)
{
    ...
    // Fetch instruction
    /* PageFault can occur here */
    
    // Get the addr from PCReg
    // Num bytes to read
    // Holds the value
    // printf("-- PCReg is %d\n", registers[PCReg]);
    ...
    instr->value = raw;
    // printf("instr value is %d\n", raw);
    instr->Decode();
    //printf("++ PCReg is %d\n", registers[PCReg]);
    //    printf("P %d Register IN MIPSIM %d\n",
    //    currentThread->space->pcb->getMyPid(),registers[PCReg]);
    if (DebugIsEnabled('m')) {
    ...
    // printf("Opcode is %d\n NextPCReg is %d\n", instr->opCode, registers[NextPCReg]);
    switch (instr->opCode) {
